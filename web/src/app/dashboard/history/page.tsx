    ...
    return (
        <div className="space-y-12">
            <header className="flex justify-between items-end">
                ...
                {selectedResult && (
                    <button onClick={() => setSelectedResult(null)} className="text-xs font-bold text-accent hover:text-white uppercase tracking-widest transition-colors flex items-center gap-2 mb-2">&larr; Back to History</button>
                )}
            </header>
                ...
                <div className="glass rounded-3xl overflow-hidden border border-white/5 shadow-2xl">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {history.map((row) => (
                                <tr key={row.ticker + row.prediction_date} onClick={() => setSelectedResult(row)} className="hover:bg-white/[0.01] transition-colors group cursor-pointer">
                                    ...
                                    <td className="px-8 py-6 font-mono font-bold text-lg text-white">{(row.confidence * 100).toFixed(0)}%</td>
