    ...
    def _format_prompt(self, company: CompanyData, news: List[NewsArticle]) -> str:
        ...
        if hasattr(company, 'company_facts') and company.company_facts:
            ...
            for fact_name, fact_data in company.company_facts.items():
                # fact_data may be a dict {"value": ..., "period_end": ..., "form": ...}
                # or a raw scalar when produced by other sources
                if isinstance(fact_data, dict):
                    val    = fact_data.get('value', 0) or 0
                    period = fact_data.get('period_end', '')
                    form   = fact_data.get('form', '')
                elif isinstance(fact_data, (int, float)):
                    val    = fact_data
                    period = ''
                    form   = ''
                else:
                    facts_str += f"  - {fact_name}: {fact_data}\n"
                    continue
                try:
                    fval = float(val)
                    ...
                    fval = 0.0
                if fval >= 1e9:
                    val_fmt = f"${fval / 1e9:.2f}B"
                elif fval >= 1e6:
                    val_fmt = f"${fval / 1e6:.2f}M"
                else:
                    val_fmt = f"${fval:,.0f}"
...
"""
    ...
    def _strip_additional_properties(schema: dict) -> dict:
        ...
        Gemini's response_schema uses its own Schema type, which does not support
        the JSON-Schema 'additionalProperties' keyword and rejects it with a 400.
        """
        import copy
    ...
    def _get_llm_kwargs(self) -> dict:
        ...
        if self.config.provider == "openai":
            ...
            kwargs["generation_config"] = {
                ...
                # Gemini's Schema type rejects 'additionalProperties' — strip it first
                "response_schema": self._strip_additional_properties(AGENT_RESPONSE_SCHEMA),
            }
        return kwargs
    ...
    def analyze(
        ...
        try:
            if stream_callback:
                ...
                # Guard: streaming can yield truncated JSON (esp. Gemini); fall back if needed
                try:
                    json.loads(full_response)
                except json.JSONDecodeError:
                    self.logger.warning(
                        "Streaming response was truncated or invalid JSON; "
...
class ThreeAgentSystem:
    ...
    async def _predict_async(
        ...
        do_rebuttals = self.enable_rebuttals
