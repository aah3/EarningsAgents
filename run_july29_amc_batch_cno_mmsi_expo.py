...
logger = logging.getLogger("AMCJuly29BatchRunner")

class CustomEarningsPipeline(EarningsPipeline):
    """
    Subclass of EarningsPipeline to force report times for specific tickers.
    Ensures CNO, MMSI, and EXPO are evaluated with AMC (After Market Close) timing context.
    """
    def predict_single(self, ticker, report_date, *args, **kwargs):
        ...
        def custom_get_company_data(t, rd, *a, **kw):
            ...
            if ticker_upper in ["CNO", "MMSI", "EXPO"]:
...
def main():
    print("=" * 70)
    print("      AI EARNINGS BATCH DEBATE RUNNER (CNO, MMSI, EXPO)")
    ...
    clerk_id = "user_july29_amc_batch_run"
    user_id = None
    try:
        with Session(engine) as session:
            ...
            if not user:
                user = User(clerk_id=clerk_id, email="amc_batch_runner@example.com")
                session.add(user)
                session.commit()
    ...
    # Clean up existing database rows for CNO, MMSI, EXPO to guarantee fresh data
    tickers_to_run = ["CNO", "MMSI", "EXPO"]
    ...
    print("\n[Step 3] Running Batch AI Prediction & Debate for CNO, MMSI, EXPO...")
    companies_data = [
        {"ticker": "CNO", "report_date": date(2026, 7, 29)},
        {"ticker": "MMSI", "report_date": date(2026, 7, 29)},
        {"ticker": "EXPO", "report_date": date(2026, 7, 29)}
    ]
