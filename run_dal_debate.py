...
logger = logging.getLogger("DeltaDebateRunner")

def main():
    print("=" * 70)
    print("         AI EARNINGS DEBATE FOR DELTA AIR LINES (DAL) - RUNNER")
    print("         Reporting Date: July 10, 2026")
    print("=" * 70)
    ...
    # Clean up any existing DAL history/calendar to guarantee a fresh fetch/sync
    print("\n[Step 1b] Cleaning up existing DAL data to ensure fresh sync...")
    with Session(engine) as session:
        session.exec(text("DELETE FROM earnings_history WHERE ticker='DAL'"))
        session.exec(text("DELETE FROM earnings_calendar_event WHERE ticker='DAL'"))
        session.commit()
    ...
    print("\n[Step 3] Running the AI Debate for DAL (fetching data + LLM debate)...")
    ...
    clerk_id = "user_dal_debate_run_2026"
    try:
        with Session(engine) as session:
            # Get or create user
            ...
            if not user:
                user = User(clerk_id=clerk_id, email="dal_debate_runner@example.com")
                session.add(user)
                session.commit()
            ...
            try:
                ...
                _refresh_profile(session, "DAL")
            except Exception as pe:
                print(f"Warning: Failed to save/refresh company profile for DAL: {pe}")
            ...
            if verified:
                print(f"Verification check: Successfully retrieved row for DAL with ID {verified.id} and Direction {verified.direction}")
