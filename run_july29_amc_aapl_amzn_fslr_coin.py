...
logger = logging.getLogger("MegaAMCJuly29BatchRunner")

class CustomEarningsPipeline(EarningsPipeline):
    """
    Subclass of EarningsPipeline to force report times for specific tickers.
    Ensures AAPL, AMZN, FSLR, and COIN are evaluated with AMC (After Market Close) timing context.
    """
    def predict_single(self, ticker, report_date, *args, **kwargs):
        ...
        def custom_get_company_data(t, rd, *a, **kw):
            ...
            if ticker_upper in ["AAPL", "AMZN", "FSLR", "COIN"]:
...
def main():
    print("=" * 70)
    print("   AI EARNINGS BATCH DEBATE RUNNER (AAPL, AMZN, FSLR, COIN)")
    print("   Reporting Date: July 29, 2026 (After Market Close - AMC)")
    print("=" * 70)
    ...
    clerk_id = "user_july29_amc_mega_batch_run"
    user_id = None
    try:
        with Session(engine) as session:
            ...
            if not user:
                user = User(clerk_id=clerk_id, email="mega_amc_batch_runner@example.com")
                session.add(user)
                session.commit()
    ...
    # Clean up existing database rows for AAPL, AMZN, FSLR, COIN to guarantee fresh data
    tickers_to_run = ["AAPL", "AMZN", "FSLR", "COIN"]
    ...
    print("\n[Step 3] Running Batch AI Prediction & Debate for AAPL, AMZN, FSLR, COIN...")
    companies_data = [
        {"ticker": "AAPL", "report_date": date(2026, 7, 29)},
        {"ticker": "AMZN", "report_date": date(2026, 7, 29)},
        {"ticker": "FSLR", "report_date": date(2026, 7, 29)},
        {"ticker": "COIN", "report_date": date(2026, 7, 29)}
    ]
    ...
    for i, company in enumerate(companies_data):
        ...
        try:
            ...
            print(f" Guidance Expectation: {pred.guidance_expectation}")
            ...
            if pred.debate_summary:
                print("-" * 80)
                print(" AGENT DEBATE SUMMARY (PASS 1):")
