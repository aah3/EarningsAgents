...
logger = logging.getLogger("PengDebateRunner")

def main():
    print("=" * 70)
    print("         AI EARNINGS DEBATE FOR PENG - RUNNER")
    print("         Reporting Date: July 7, 2026 (After close)")
    print("=" * 70)
    ...
    # Clean up any existing PENG predictions
    print("\n[Step 1b] Cleaning up existing PENG data to ensure fresh run...")
    try:
        with Session(engine) as session:
            session.exec(text("DELETE FROM prediction WHERE ticker='PENG'"))
            session.commit()
        print("Clean up completed.")
    except Exception as e:
        print(f"Clean up skipped or failed: {e}")
    ...
    print("\n[Step 3] Running the AI Debate for PENG (fetching data + LLM debate)...")
    ...
    clerk_id = "user_peng_debate_run_2026"
    try:
        with Session(engine) as session:
            # Get or create user
            ...
            if not user:
                user = User(clerk_id=clerk_id, email="peng_debate_runner@example.com")
                session.add(user)
                session.commit()
            ...
            try:
                ...
                _refresh_profile(session, "PENG")
            except Exception as pe:
                print(f"Warning: Failed to save/refresh company profile for PENG: {pe}")
            ...
            if verified:
                print(f"Verification check: Successfully retrieved row for PENG with ID {verified.id} and Direction {verified.direction}")
