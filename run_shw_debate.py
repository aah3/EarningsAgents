...
logger = logging.getLogger("SHWDebateRunner")

def main():
    print("=" * 70)
    print("         AI EARNINGS DEBATE FOR SHERWIN-WILLIAMS (SHW) - RUNNER")
    print("======================================================================")
    ...
    # Clean up existing SHW data to guarantee fresh fetch/sync
    print("\n[Step 1b] Cleaning up existing SHW data to ensure fresh sync...")
    with Session(engine) as session:
        session.exec(text("DELETE FROM earnings_history WHERE ticker='SHW'"))
        session.exec(text("DELETE FROM earnings_calendar_event WHERE ticker='SHW'"))
        session.commit()
    ...
    print("\n[Step 3] Running the AI Debate for SHW (fetching data + LLM debate)...")
    ...
    try:
        with Session(engine) as session:
            ...
            print(f"Successfully saved prediction for SHW (ID: {db_prediction.id}) to database.")
    except Exception as e:
    ...
    pipeline.shutdown()
    print("\n" + "=" * 70)
    print(f"       SHW AI EARNINGS DEBATE COMPLETE: PREDICTION = {prediction.direction.value.upper()}")
    print("=" * 70)
