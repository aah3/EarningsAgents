...
logger = logging.getLogger("PYPLDebateRunner")

def main():
    print("=" * 70)
    print("         AI EARNINGS DEBATE FOR PAYPAL HOLDINGS (PYPL) - RUNNER")
    print("         Reporting Date: July 28, 2026 (Before Market Open)")
    print("=" * 70)
    ...
    # Clean up any existing PYPL history/calendar to guarantee a fresh fetch/sync
    print("\n[Step 1b] Cleaning up existing PYPL data to ensure fresh sync...")
    with Session(engine) as session:
        session.exec(text("DELETE FROM earnings_history WHERE ticker='PYPL'"))
        session.exec(text("DELETE FROM earnings_calendar_event WHERE ticker='PYPL'"))
        session.commit()
    ...
    print("\n[Step 3] Running the AI Debate for PYPL (fetching data + LLM debate)...")
    ...
    try:
        with Session(engine) as session:
            ...
            print(f"Successfully saved prediction for PYPL (ID: {db_prediction.id}) to database.")
    except Exception as e:
    ...
    pipeline.shutdown()
    print("\n" + "=" * 70)
    print(f"       PYPL AI EARNINGS DEBATE COMPLETE: PREDICTION = {prediction.direction.value.upper()}")
    print("=" * 70)
