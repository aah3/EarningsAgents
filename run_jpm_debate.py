...
logger = logging.getLogger("JPMDebateRunner")

def main():
    print("=" * 70)
    print("         AI EARNINGS DEBATE FOR JPMORGAN CHASE & CO. (JPM) - RUNNER")
    ...
    # Clean up any existing JPM history/calendar to guarantee a fresh fetch/sync
    print("\n[Step 1b] Cleaning up existing JPM data to ensure fresh sync...")
    with Session(engine) as session:
        session.exec(text("DELETE FROM earnings_history WHERE ticker='JPM'"))
        session.exec(text("DELETE FROM earnings_calendar_event WHERE ticker='JPM'"))
        session.commit()
    ...
    print("\n[Step 3] Running the AI Debate for JPM (fetching data + LLM debate)...")
    ...
    clerk_id = "user_jpm_debate_run_2026"
    try:
        with Session(engine) as session:
            # Get or create user
            ...
            if not user:
                user = User(clerk_id=clerk_id, email="jpm_debate_runner@example.com")
                session.add(user)
                session.commit()
            ...
            try:
                ...
                _refresh_profile(session, "JPM")
            except Exception as pe:
                print(f"Warning: Failed to save/refresh company profile for JPM: {pe}")
            ...
            if verified:
                print(f"Verification check: Successfully retrieved row for JPM with ID {verified.id} and Direction {verified.direction}")
